...
if($action == 'check_user') {
    ...
    if(!empty($id)) {
        $dup = checkDuplicateFields("users", ["email" => $email], ["user_id" => $id]);
    }else{
        $dup = checkDuplicateFields("users", ["email" => $email]);
    }
...
if($action == "verify_otp") {
    ...
    if(isset($_POST['duplicate_id']) && !empty($_POST['duplicate_id'])) {
        $user = select("SELECT * FROM users WHERE email = ?", [$_SESSION['email_verified']], "s");
        $data = $user['data'][0];
    }else{
        $data = $_SESSION['email_verified'];
        $data = [
    ...
    unset($_SESSION['email_otp']);
    unset($_SESSION['email_opt_expiry']);
    json_response("success", "Email verification successfully.", $data);
