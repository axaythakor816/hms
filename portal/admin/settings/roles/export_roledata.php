...
$sql = "SELECT " . implode(", ", $selectFields) . " FROM roles WHERE 1";
$whereValues = [];
$searchType = "";
...
foreach ($result['data'] as &$row) {
    if(isset($row['role_name'])) $row['role_name'] = ucfirst(strtolower($row['role_name']));
...
if(!empty($type)) {
    switch (strtolower($type)) {
        case 'csv':
            exportCSV($result['data'], "Roles.csv");
            break;
        case 'txt':
            exportTXT($result['data'], "Roles.txt");
            break;
        case 'pdf':
            exportPDF($result['data'], "Roles.pdf");
            break;
        case 'xlsx':
            exportXLSX($result['data'], "Roles.xlsx");
            break;
        default:
